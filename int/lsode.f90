...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      ...
      KPP_REAL :: RCNTRL(20), RSTATUS(20)
      KPP_REAL :: RPAR(1)
      KPP_REAL :: H_Start, H_Max, H_Min, H_Exit

      INTEGER       :: ICNTRL(20), ISTATUS(20)
      INTEGER       :: IPAR(1), ITOL, ITASK, IERR, IOPT, MF
      INTEGER       :: Max_Steps,  Max_Order
      INTEGER       :: N_FuncEval, N_JacEval, N_Steps
      ...
      Max_Steps  = ICNTRL(4)         ! Max number of internal steps
      Max_Order  = ICNTRL(10)        ! Max order
      H_Min      = RCNTRL(1)         ! Low bound of integration step size
      H_Max      = RCNTRL(2)         ! High bound of integration step size
      H_Start    = RCNTRL(3)         ! Starting value of integration step size
      ITOL       = 1                 ! Abs/RelTol are both scalars
      IF ( ICNTRL(2) ==0 ) ITOL = 4  ! Abs/RelTol are both vectors

      ! Call the LSODE integrator with keyword arguments for clarity
      ! (Bob Yantosca, 08 Jan 2025)
      CALL DLSODE( F          = FUN_CHEM,                                    &
                   NEQ        = NVAR,                                        &
                   Y          = Y,                                           &
                   T          = TIN,                                         &
                   TOUT       = TOUT,                                        &
                   ...
                   H_Max      = H_Max,                                       &
                   H_Start    = H_Start,                                     &
                   N_FuncEval = N_FuncEval,                                  &
                   N_JacEval  = N_JacEval,                                   &
                   N_Steps    = N_Steps,                                     &
                   H_Exit     = H_Exit                                      )

      ! Copy outputs to ISTATUS and RSTATUS
      ! (Bob Yantosca (08 Jan 2025)
      ISTATUS(1) = N_FuncEval  ! Number of function evaluations
      ISTATUS(2) = N_JacEval   ! Number of Jacobian evaluations
      ISTATUS(3) = N_Steps     ! Number of steps
      RSTATUS(1) = TOUT        ! Time corresponding to computed Y at return
      RSTATUS(2) = H_Exit      ! Last accepted step before exit
...
!DECK DLSODE
      !
      ! Refactor subroutine DLSODE to make RWORK and IWORK local arrays
      ! in order to avoid compilation errors with OpenMP parallelization.
      ! (Bob Yantosca, 08 Jan 2025)
      SUBROUTINE DLSODE( F,       NEQ,    Y,       T,          TOUT,         &
                         ITOL,    RelTol, AbsTol,  ITASK,      ISTATE,       &
                         IOPT,    JAC,    MF,      Max_Steps,  Max_Order,    &
                         H_Min,   H_Max,  H_Start, N_FuncEval, N_JacEval,    &
                         N_Steps, H_Exit                                    )

      ! Added comments (Bob Yantosca, 08 Jan 2025)
      EXTERNAL :: F              ! Function to be integrated
      EXTERNAL :: JAC            ! Jacobian function
      INTEGER  :: NEQ            ! Number of equations
      INTEGER  :: ITOL           ! Tolerances are: 1=scalar; 4=vector
      INTEGER  :: ITASK          ! 1=normal computation
      INTEGER  :: ISTATE         ! Flag for state of calculation
      INTEGER  :: IOPT           ! Are optional inputs used? (0=no; 1=yes)
      INTEGER  :: MF             ! Method flag (21=full Jacobian)
      INTEGER  :: Max_Steps      ! Max number of integration steps
      INTEGER  :: Max_Order      ! Maximal order of integration method
      INTEGER  :: N_FuncEval     ! Number of times FUN was evaluated
      INTEGER  :: N_JacEval      ! Number of times JAC was evaluated
      INTEGER  :: N_Steps        ! Number of integration steps
      KPP_REAL :: Y(:)           ! Concentration vector Y
      KPP_REAL :: T              ! Starting time of integration step
      KPP_REAL :: TOUT           ! Ending time of integration step
      KPP_REAL :: RelTol(:)      ! Relative tolerances
      KPP_REAL :: AbsTol(:)      ! Absolute tolerances
      KPP_REAL :: H_Min          ! Lower bound on integration step size
      KPP_REAL :: H_Max          ! Upper bound on integration step size
      KPP_REAL :: H_Start        ! Starting value of integration step size
      KPP_REAL :: H_Exit         ! Last accepted time step value

      ! Declare RWORK and IWORK as local arrays (Bob Yantosca, 08 Jan 2025)
      KPP_REAL :: RWORK(LRW)     ! Reals
      INTEGER  :: IWORK(LIW)     ! Integers
...
!-----------------------------------------------------------------------
      COMMON /DLS001/ ROWNS(209),                                      &
        ...
        MAXORD, MAXCOR, MSBP, MXNCF, N, NQ, NST, NFE, NJE, NQU
      !
      ! Hold common block private for OpenMP (Bob Yantosca, 08 Jan 2025)
...
!-----------------------------------------------------------------------
!
      ! Copy integer inputs into IWORK (Bob Yantosca, 08 Jan 2025)
      IWORK(1:30) = 0
      IWORK(5)    = Max_Order
      IWORK(6)    = Max_Steps

      ! Copy real inputs into RWORK (Bob Yantosca, 08 Jan 2025)
      RWORK(1:30) = 0.0d0
      RWORK(5)    = H_Start
      RWORK(6)    = H_Max
      RWORK(7)    = H_Min
...
!-----------------------------------------------------------------------
  ...
  420 ISTATE = 2
      ...
      ! Pass integrator statistics back via new output arguments
      ! (Bob Yantosca, 08 Jan 2025)
      H_Exit     = HU
      N_FuncEval = NFE
      N_JacEval  = NJE
...
!$$ Comment out unused routine DSRCOM (Bob Yantosca, 08 Jan 2025)
!$$!DECK DSRCOM
