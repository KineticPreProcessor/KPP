...
for this_test in ${GENERAL_TESTS}; do
    run_ci_test "${this_test}" "${this_test}" "${cwd}" ""
done
...
if [[ "x${DO_MCM}" == "x1" ]]; then
  run_ci_test "${MCM_1}" "mcm" "${cwd}" "EXTERNAL_RATES_F90=constants_mcm.f90"
  run_ci_test "${MCM_2}" "mcm" "${cwd}" "EXTERNAL_RATES_F90=constants_mcm.f90"
fi
