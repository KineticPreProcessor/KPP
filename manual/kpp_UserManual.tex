\def\myfiledate{2022-04-29} % "\myfiledate" because packages may change \filedate
...
%-----------------------------------------------------------------------------
\subsubsection{Return equation rates from the {\tt Fun()} routine
  ({\tt\#RETURNRATES})}
\label{sec:command-returnrates}
%-----------------------------------------------------------------------------

In some modeling contexts, it is desirable to return the equation rates
for diagnostic purposes. The \code{#RETURNRATES} switch allows you to
return the equation rates (the \code{A} variable in solver routine
\code{Fun}) via the optional \code{Aout} variable.
