\def\myfiledate{2022-04-27} % "\myfiledate" because packages may change \filedate
...
\code{#RETURNRATES}  & \ref{sec:command-returnrates}\\
\hhline
\end{tabular}
\end{center}
\end{table}
...
You may restrict a chemical mechanism to use a given version of KPP or later.
to do this, add \code{#MINVERSION x.y.z} to the definition file.  The version
number \{x.y.z} adheres to the Semantic Versioning style, where \code{x} is
the major version number, \code{y}, is the minor version number, and
\code{z} is the bugfix (aka "patch") version number.
...
%-----------------------------------------------------------------------------
\subsubsection{Return equation rates from the \code{Fun()} routine ({\tt\#RETURNRATES})}
\label{sec:command-returnrates}
%-----------------------------------------------------------------------------

In some modeling contexts, it is desirable to return the equation rates for
diagnostic purposes.  The \code{#RETURNRATES} switch allows you to return
the equation rates (the \code{A}) variable in solver routine \code{Fun})
via the optional \code{Aout} variable.
...
{\it command} ::=  & \code{#CHECKALL}                                                    & $|$\\
                   ...
                   & \code{#TRANSPORTALL}                                                & $|$\\
                   & \code{#UPPERCASEF90} \verb![ ON | OFF ]!                            & $|$\\
                   & \code{#MINVERSION} {\it minimum_version_number}                     & $|$\\
		   & \code{#RETURNRATES} \verb![ ON | OFF ]!                             &\\[3mm]
