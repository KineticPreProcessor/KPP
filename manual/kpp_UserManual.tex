\def\myfiledate{2022-05-04} % "\myfiledate" because packages may change \filedate
...
\input{kpp_info.tex} % \def\kppversion{...}
...
\begin{center}
  ...
  {\Huge\bf KPP-\kppversion\ User's Manual}\\[9mm]
  ...
  {\large This manual is available at:
    \url{https://github.com/KineticPreProcessor/KPP/blob/main/manual/kpp_UserManual.pdf}}
...
later. To do this, add \code{#MINVERSION x.y.z}. The version number
\code{x.y.z} adheres to the Semantic Versioning style
(\url{https://semver.org}), where \code{x} is the major version number,
\code{y} is the minor version number, and \code{z} is the bugfix (aka
...
If you have selected the \code{#LANGUAGE Fortran90} option, KPP will
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Information for KPP Developers}
\label{sec:developer-info}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\subsection{Adding new KPP commands}
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

To add a new KPP command, the source code has to be edited at several
locations. A short summary is presented here, using the new command
\code{#NEWCMD} as an example:

\begin{itemize}
\item Add \code{#NEWCMD} to several files in the \code{src} directory:
  \begin{itemize}
  \item \code{scan.h}: \verb|void CmdNEWCMD( char *cmd );|
  \item \code{scan.l}: \verb|{ "NEWCMD", PRM_STATE, NEWCMD },|
  \item \code{scanner.c}: \verb|void CmdNEWCMD( char *cmd )|
  \item \code{scan.y}:
    \begin{itemize}
    \item \verb|%token NEWCMD|
    \item \verb|NEWCMD PARAMETER|
    \item \verb|{ CmdNEWCMD( $2 ); }|
    \end{itemize}
  \end{itemize}
\item Maybe add a CI-test:
  \begin{itemize}
  \item Create a new directory \code{ci-tests/ros_newcmd}
  \item Add new CI-test to \code{ci-manual-testing-script.sh} and
    \code{ci-testing-script.sh} in the \code{.ci-pipelines} directory
  \end{itemize}
\item Other:
  \begin{itemize}
  \item Explain in user manual \code{manual/kpp_UserManual.tex}:
    \begin{itemize}
    \item Add to table \code{tab:commands}
    \item Add a section \code{sec:command-newcmd}
    \item Add to BNF description table \code{sec:bnf}
    \end{itemize}
  \item Add to \code{site-lisp/kpp.el}
  \end{itemize}
\end{itemize}
...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\subsection{KPP-2.5.0}
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

\todo{add text here}

%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\subsection{KPP-2.4.0}
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

\todo{add text here}

%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
\subsection{KPP-2.2.3}
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

%-----------------------------------------------------------------------------
\subsubsection{New features of KPP-2.2.3}
%-----------------------------------------------------------------------------
